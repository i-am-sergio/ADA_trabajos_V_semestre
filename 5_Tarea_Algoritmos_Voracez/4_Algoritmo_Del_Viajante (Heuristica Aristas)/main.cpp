...
public:
  ...
  void viajante_BasadoEnAristas(V origen, vector<Edge<V, A> *> &solucionAristas)
{
  Vertex<V, A> *nodoActual = &(*find_Vertex(origen));
  nodoActual->m_Visit = true;

  while (solucionAristas.size() < m_Graph.size() - 1)
  {
    Edge<V, A> *aristaMinima = nullptr;
    int menor = INF;

    // Buscar la arista de menor costo que conecte un nodo visitado con un nodo no visitado
    for (auto &edge : nodoActual->m_list_Edge)
    {
      if (edge.m_pVertexDestino->m_Visit && !edge.m_pVertexDestino->m_Visit && edge.m_Dato < menor)
      {
        aristaMinima = &edge;
        menor = edge.m_Dato;
      }
    ...
    // Si se encontró una arista válida, agregarla a la solución
    if (aristaMinima)
    {
      solucionAristas.push_back(aristaMinima);
      aristaMinima->m_pVertexDestino->m_Visit = true;
      nodoActual = aristaMinima->m_pVertexDestino;
    }
  }

  for (auto &edge : nodoActual->m_list_Edge)
  {
    if (edge.m_pVertexDestino->m_Dato == origen)
    {
      solucionAristas.push_back(&edge);
      break;
    }
  }

  for (auto &edgePtr : solucionAristas)
  {
    cout << edgePtr->m_pVertexOrigen->m_Dato << " - " << edgePtr->m_pVertexDestino->m_Dato << " ";
  }
  cout << endl;
}
...
int main()
{
  ...
  cout << "\n----- Resultado Viajante -----\n";
  vector<Edge<string, int>*> solucionAristas;
  G.viajante_BasadoEnAristas("5", solucionAristas);
